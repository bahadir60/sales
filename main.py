...
# SAYFA AYARLARI
# ---------------------------------------------------------
st.set_page_config(page_title="E-Ticaret Paneli V15.1", layout="wide", page_icon="🔒")
...
IS_ADMIN = False
if admin_password == "admin123": # ŞİFRE BURADA
    IS_ADMIN = True
    st.sidebar.success("✅ Yönetici Modu")
else:
    st.sidebar.info("👁️ İzleyici Modu")

st.sidebar.markdown("---")
st.sidebar.title("🎛️ Menü")

if IS_ADMIN:
    menu = st.sidebar.radio("Seçim:", ["📊 Dashboard", "📤 Excel Yükle (Aylık)", "📝 Manuel Giriş (Haftalık/Aylık)", "⚙️ Ayarlar"])
else:
    menu = st.sidebar.radio("Seçim:", ["📊 Dashboard"])
...
if menu == "📊 Dashboard":
    ...
    if df.empty:
        ...
        if comp_mode != "Yok":
            ...
            with c2:
                if not df_curr.empty:
                    ...
                    # DÜZELTME: Girinti hatası burada giderildi
        ...
        if IS_ADMIN:
            ...
            st.info("ℹ️ Sadece görüntüleme modundasınız. Veri düzenlemek için yönetici girişi yapınız.")

# ---------------------------------------------------------
# MODÜL 2: EXCEL YÜKLEME (Sadece Admin)
# ---------------------------------------------------------
    ...
    if uploaded_file:
        try:
            ...
            for i, row in df_temp.head(10).iterrows():
                row_list = row.astype(str).str.lower().tolist()
                if any('firm' in s for s in row_list):
                    h_row = i + 1
                    break
...
# ---------------------------------------------------------
# MODÜL 3: MANUEL GİRİŞ (Sadece Admin)
# ---------------------------------------------------------
    ...
    with st.form("man"):
        ...
        if st.form_submit_button("Kaydet"):
            row = {'Tarih': pd.to_datetime(d), 'Firma': f, 'Ulke': cnt, 'Sales': s, 'Unit': u, 'AdsSpend': sp, 'AdsSales': asl, 'AdsOrder': ao, 'VeriTipi': c_type}
            update_database(pd.DataFrame([row]))
            st.success("Kaydedildi.")

# ---------------------------------------------------------
# MODÜL 4: AYARLAR (Sadece Admin)
# ---------------------------------------------------------
elif menu == "⚙️ Ayarlar":
    st.title("⚙️ Ayarlar")
